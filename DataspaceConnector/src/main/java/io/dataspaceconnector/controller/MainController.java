...
public class MainController {
    ...
    public ResponseEntity<Object> getPrivateSelfDescription() {
        String rdf = connectorService.getConnectorWithOfferedResources().toRdf();
        if (CheckRDF.checkRDF("baseids", rdf))
        {
    ...
    @GetMapping(value = "/api/connectorbad", produces = ContentType.JSON_LD)
    @Operation(summary = "Get the private IDS self-description bad.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = ResponseCode.OK, description = ResponseDescription.OK),
            @ApiResponse(responseCode = ResponseCode.INTERNAL_SERVER_ERROR,
                    description = ResponseDescription.INTERNAL_SERVER_ERROR)})
    @ResponseBody
    @TelemetrySpan(name = "GET /api/connectorbad")
    public ResponseEntity<Object> getPrivateSelfDescriptionBad() {
        BaseConnector conn = connectorService.getConnectorWithOfferedResources();
        String rdf = conn.toRdf();
        rdf = rdf.replace("hasDefaultEndpoint", "test");
        if (CheckRDF.checkRDF("baseids", rdf))
        {
                return ResponseEntity.ok(rdf);
        }
...
//     @GetMapping(value = "/api/rdfresources", produces = ContentType.JSON_LD)
//     @Operation(summary = "Get resources as rdf")
//     @ApiResponses(value = {
//             @ApiResponse(responseCode = ResponseCode.OK, description = ResponseDescription.OK),
//             @ApiResponse(responseCode = ResponseCode.INTERNAL_SERVER_ERROR,
//                     description = ResponseDescription.INTERNAL_SERVER_ERROR)})
//     @ResponseBody
//     @TelemetrySpan(name = "GET /api/rdfresources")
//     public ResponseEntity<Object> getRDFResources() {
//         Entity = OfferedResourceController.
//     }
